...
import { UPDATE_TENANT_FORM_FIELDS, UPDATE_TENANT_FORM_SCHEMA } from '@/constants'
import type { ITenantFormProps, UPDATE_TENANT_FORM_SCHEMA_TYPE } from '@/types'
  ...
  useEffect(() => {
    if (defaultValues) {
      form.reset({
        ...
        isActive: defaultValues.isActive ? 'active' : 'inactive',
      })
    }
  }, [defaultValues, form])
  ...
  return (
    <Form {...form}>
      <form className="space-y-4" onSubmit={form.handleSubmit(onSubmit)}>
        {/* Task Detail */}
        {UPDATE_TENANT_FORM_FIELDS.map(({ title, description, fields }) => (
